...
export class NavChaser extends Navigator {
    ...
    protected onTransitionFinish(): void {
        ...
        // 1. 找出玩家最近的 waypoint
        let targetClosest: Waypoint = null;
        let minDist = Infinity;

        for (let wp of this.waypointGraph.adjacencyList) {
            const dist = wp.distanceToNode(this._runTowards);
            if (dist < minDist) {
                minDist = dist;
                targetClosest = wp;
            }
        ...
        // 2. 從目前 waypoint 移動到目標 waypoint 的下一個 waypoint
        const next = this.waypointGraph.shortestPathMatrix.get(
            this.currentWaypoint.uuid + targetClosest.uuid
        );
