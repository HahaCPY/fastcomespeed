    ...
    private allPairsShortestPath() {
        ...
        for (let A of this.adjacencyList) {
            for (let B of this.adjacencyList) {
                ...
                if (A === B) {
                    this._shortestDistanceMatrix.set(key, 0);
                    this._shortestPathMatrix.set(key, A);
                } else {
                    const index = A.adjacentWaypoints.indexOf(B);
                    if (index !== -1) {
                        const dist = A.distances[index];
                        this._shortestDistanceMatrix.set(key, dist);
                        this._shortestPathMatrix.set(key, B);
                    } else {
                        this._shortestDistanceMatrix.set(key, Infinity);
                        this._shortestPathMatrix.set(key, null);
                    }
                }
            }
        }
        ...
        //	   and shortestPathMatrix(A, B) is shortestPathMatrix(A, C).
        // END PSEUDOCODE 
        for (let C of this.adjacencyList) {
            for (let A of this.adjacencyList) {
                for (let B of this.adjacencyList) {
                    const abKey = A.uuid + B.uuid;
                    const acKey = A.uuid + C.uuid;
                    const cbKey = C.uuid + B.uuid;
    
                    const distAB = this._shortestDistanceMatrix.get(abKey);
                    const distAC = this._shortestDistanceMatrix.get(acKey);
                    const distCB = this._shortestDistanceMatrix.get(cbKey);
    
                    if (distAC + distCB < distAB) {
                        this._shortestDistanceMatrix.set(abKey, distAC + distCB);
                        this._shortestPathMatrix.set(abKey, this._shortestPathMatrix.get(acKey));
                    }
                }
            }
        }
